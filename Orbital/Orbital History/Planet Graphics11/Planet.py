...
explodedy=[]
for n in range(30):
    explodex.append(n); explodex[n]=0
    explodey.append(n); explodey[n]=0
    explodedx.append(n); explodedx[n]=random.randint(-8,8)/10
    explodedy.append(n); explodedy[n]=random.randint(-8,8)/10
...
def crashedRocketship():
    for n in range(30):
        if (explodex[n]==0): explodex[n]=WIDTH/2+explodedx[n]
        if (explodey[n]==0): explodey[n]=HEIGHT/2+explodedy[n]
        explodex[n]=explodex[n]+explodedx[n]
        explodey[n]=explodey[n]+explodedy[n]
        pygame.draw.line(screen, WHITE, (explodex[n],explodey[n]), (explodex[n]+explodedx[n]*20,explodey[n]+explodedy[n]*20))
...
def controlRocketship():
    ...
    #Planet Collision
    if (c<landy+38): #Crashes
        d=atan(b/a)
        d=d-spin
        ang=abs(rrot-d)
        if (ang>3.14): ang=ang-3.14
        if (abs(dx+dy)>4 or ang<1.57-.7 or ang>1.57+.7): crashed=1
        if (abs(dx+dy)>.7): dx=-dx; dy=-dy
        
        if (abs(dx+dy)<=.7):
            if ((c<landy+38) and (engine==0 or landed==0)): #Landed
                landed=1
                dx=0; dy=0
                if (a>=0):
                ...
                if (a<0):
                    if (ang<1.57-.05): rrot=rrot+.03
                    if (ang>1.57+.05): rrot=rrot-.03
                if (a<0): rx=w+cos(d)*(landy+18+20); ry=h+sin(d)*(landy+18+20)
                if (a>=0): rx=w-cos(d)*(landy+18+20); ry=h-sin(d)*(landy+18+20)

                    
    if (c>landy+38): landed=0
