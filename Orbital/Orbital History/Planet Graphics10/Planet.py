...
def controlRocketship():
    ...
    #Planet Collision
    aa=w-rx; bb=h-ry; cc=(aa*aa+bb*bb)**.5
    ...
    if (cc<landy+38):
        ...
        d=d-spin
        if (a>=0):
            if (rrot-d<-1.57-.7 or rrot-d>-1.57+.7): crashed=1
        if (a<0):
            if (rrot-d<1.57-.7 or rrot-d>1.57+.7): crashed=1
        if (abs(dx+dy)>.7): dx=-dx; dy=-dy
        
        if (abs(dx+dy)<=.7):
            if ((cc<landy+38) and (engine==0 or landed==0)): #Landed
                landed=1
                dx=0; dy=0
                if (a<0): rrot=d+3.14/2; rx=w+cos(d)*(landy+18+20); ry=h+sin(d)*(landy+18+20)
                if (a>=0): rrot=d-3.14/2; rx=w-cos(d)*(landy+18+20); ry=h-sin(d)*(landy+18+20)
    if (cc>=landy+38): landed=0
...
while mainloop:
    ...
    pygame.display.set_caption("FPS: "+str(int(Clock.get_fps()))+" "+str(rrot))
    pygame.display.flip()
